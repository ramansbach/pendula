## pendula
[![Build Status](https://travis-ci.org/uwescience/pendula.svg?branch=master)](https://travis-ci.org/uwescience/pendula)

Pendula is a modified form of pendula (https://github.com/uwescience/shablona), a template project for small scientific python projects. The
...
button on the front page of the "pendula" repository.
...
`pendula/pendula.py` to be called `smallish/smallish.py`

	git mv pendula smallish
	git mv smallish/pendula.py smallish/smallish.py
	git mv smallish/tests/test_pendula.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `pendula` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `pendula` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `pendula` in them. Type
the following to see where all these files are:

	git grep pendula

You can replace `pendula` for `smallish` quickly with:

	git grep -l 'pendula' | xargs sed -i 's/pendula/smallish/g'
...
Edit all the mentions of `pendula` in the `pendula/__init__.py` file, and in
the `pendula/version.py` file as well.
...
`pendula/pendula.py`. The code provided in this file is intentionally rather
...
In the case of the pendula module, the main classes defined at the bottom of
...
(typically, something like our `pendula/tests/test_pendula.py`). Within each
...
have one such test in `pendula/tests/test_pendula.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/pendula`),
and type:
...
will see a message such as:
    ...
    pendula/tests/test_pendula.py .F...
    ...
    pendula/tests/test_pendula.py:49: AssertionError
...
To document `pendula` we use the [sphinx documentation
...
`pendula` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `pendula`, we use the
...
[here](https://github.com/uwescience/pendula/blob/master/scripts/Figure1.ipynb).
